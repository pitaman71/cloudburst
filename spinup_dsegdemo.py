...
run('%(cloudburstHome)s/cli.py launch %(name)s DSEGClusterDemo agent.clusterName=%(name)s agent.dseCreds=AlanDatastaxCredentials agent.ec2Creds=AlanExperoOnEC2 agent.config=DSEGDemoClusterConfig' % args)
